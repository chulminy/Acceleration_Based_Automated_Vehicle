...
#define b		plhs[3] 			// b
	...
	const double *pfeatureVec = mxGetPr(featureVec);
	...
	std::vector<double> vec_x(Npt_total);
	std::vector<double> vec_z(pfeatureIdx, pfeatureIdx + Npt_total), vec_z_org(pfeatureIdx, pfeatureIdx + Npt_total);
	std::vector<double> vec_w(pw, pw + Npt_total), vec_w_org(pw, pw + Npt_total);
	...
	double sum_w = 0.0, Ezw = 0.0, sum_w_z_2 = 0.0;
	...
	for (int tt = 0; tt < Nrounds; tt++)
	{
		...
		{
			// Npt_total = Number of thresholds that we will consider
			vec_x.assign(pfeatureVec + Npt_total*qq, pfeatureVec + Npt_total*(qq + 1));
		...
		sort(error_vectors, error_vectors, vec_i);
