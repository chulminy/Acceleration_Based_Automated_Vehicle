function [k, th, a, b, Fx, w] = GentleBoost_original(featureVec, featureIdx, Nrounds)
...
for m = 1:Nrounds
    ...
    % f_m = a * (x_k > th) + b
    [k(m), th(m), a(m), b(m),error] = selectBestRegressionStump(featureVec, featureIdx, w);
    ...
    find(and(sign(featureIdx),sign(fm)) ==-1)
end
